...
def _search_you(query: str, max_results: int) -> str:
    ...
    with start_span(
        ...
        type=SpanTypeAttribute.TOOL,
        input={"query": query, "max_results": max_results},
        metadata={"provider": "you"},
    ) as tool_span:
        with urlopen(request, timeout=30) as response:
            payload = json.loads(response.read().decode("utf-8"))
        output = _build_you_output(payload, max_results=max_results)
        tool_span.log(output=output)
        return output
...
def tavily_search(query: str, max_results: int = 3) -> str:
    ...
    with start_span(
        ...
        metadata={"provider_order": _search_provider_order()},
    ) as tool_span:
        errors: list[str] = []
        for provider in _search_provider_order():
            try:
                output = _search_with_provider(
                    provider=provider,
                    query=query,
                ...
                tool_span.log(
                    output=output,
                    metadata={"provider": provider, "provider_errors": errors},
                )
                return output
